...
select max(TotalAmount) as[Max_amount] from dbo.Sales;

select max(SaleDate) as [Max_salesDate]  from dbo.Sales;

select max(PaymentMethod) as [Pay_method] from dbo.Sales;

-----max Quantity sold for each product id--

select ProductID,max(Quantity) as[Maximum Quantity] from dbo.Sales
group by ProductID;

--maxmimum total amount for all distinct date in salles column
